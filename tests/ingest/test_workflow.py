...
def test_chain_from_parsing_to_storage_completes_task(eager_celery):
    ...
    from ingest.workflow import dispatch_stage
...
def test_cancelled_task_is_not_advanced():
    ...
    ``ingest.workflow.cancel_task`` 里的 revoke 只是尽力而为（需要 worker 在线），
    真正兜底的是信号里的这个检查。
    """
...
def test_defer_pipeline_skips_sync_pipeline(monkeypatch):
    ...
    calls = []
    monkeypatch.setattr("ingest.pipeline.run_config_pipeline", lambda device, config: calls.append(config.pk))
...
def test_config_job_chain_gives_both_tasks_the_same_id():
    ...
    from ingest.tasks import run_config_parsing, run_config_storage

    sig = chain(run_config_parsing.si(7), run_config_storage.si(7))

    assert [task.task for task in sig.tasks] == ["ingest.run_config_parsing", "ingest.run_config_storage"]
