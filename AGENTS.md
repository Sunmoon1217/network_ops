...
├── tmp/
├── docker/entrypoint.sh # 应用容器入口：静态成品复制进命名卷（空目录挂载点）；gunicorn 整条命令在这里拼（默认值 + GUNICORN_* 环境变量）
