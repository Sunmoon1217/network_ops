...
三者的映射全靠命名约定（模板的顶层 group 名 = config_json 的键 = Saver 的 keys），
缺少校验时只会**静默失效**：模板改了结构、或 Saver 注册到了错误的设备类型上，
都不会报错，只是数据不再入库。

本文件把这份约定固化下来。下面的缺口清单是**快照**：
修复缺口后需要同步更新常量，否则测试会失败并提醒。
"""

from ops.parsers.contract import KNOWN_MISSING_PRODUCER, KNOWN_UNCONSUMED_PRODUCTS
from ops.parsers.factory import ParserFactory
from ops.parsers.template_keys import template_keys
...
def _products_by_device_type() -> dict[str, set[str]]:
    ...
    for (_vendor, device_type), parser_cls in ParserFactory._registry.items():
        products.setdefault(device_type, set()).update(parser_cls.provides_keys)
    return products


def _consumers_by_device_type() -> dict[str, set[str]]:
    """按设备类型聚合所有 Saver 消费的键"""
    consumed: dict[str, set[str]] = {}
    for device_type, key in all_savers():
        consumed.setdefault(device_type, set()).add(key)
    return consumed
