...
const virtuals = ref<VirtualConfig[]>([])
...
// 获取接口配置
async function fetchVirtuals(page = 1) {
  loading.value = true
  error.value = null
  try {
    // 构建查询参数
    ...
    // 发送请求
    const data = await api.get('/virtuals/', { params })

    console.log('API响应:', data)

    // 处理响应数据
    virtuals.value = data.data?.results || []
    ...
    error.value = e instanceof Error ? e.message : '获取VS配置失败'
  } finally {
    loading.value = false
  }
}
...
function resetFilters() {
  ...
  ipFilter.value = ''
  fetchVirtuals(1) // 重置后重新获取数据，回到第一页
}
...
<template>
  <div class="virtual-management-container">
    <h2>接口管理</h2>
    ...
    <div v-else-if="error" class="error">
      <p>错误: {{ error }}</p>
      <button @click="fetchVirtuals(currentPage)" class="retry-btn">重试</button>
    </div>
