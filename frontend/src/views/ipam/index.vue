...
<template>
  <PageLayout title="IP 管理">
    ...
    <div class="table-wrapper">
      <DataTable :table-key="TABLE_KEYS.ipamSubnets" :data="subnets" :loading="loading">
        <DataColumn prop="network" label="网段" width="160" sortable />
        <DataColumn prop="gateway" label="网关" width="140" />
        <DataColumn prop="vlan" label="VLAN" width="80" />
        <DataColumn prop="tag_names" label="标签" width="150">
          <template #default="{ row }"><el-tag v-for="t in row.tag_names" :key="t" size="small" style="margin-right: 4px">{{ t }}</el-tag></template>
        </DataColumn>
        <DataColumn prop="total_ips" label="总IP" width="80" />
        <DataColumn prop="used_ips" label="已用" width="70" />
        <DataColumn prop="utilization" label="使用率" width="90">
